...
public class SeptemberPage extends BasePage {
    ...
    @FindBy(xpath = "//h2[contains(text(),'September')]")
    WebElement septemberShoesPage;
    ...
    public SeptemberPage(WebDriver driver) {
        super(driver);
        this.driver = driver;
        PageFactory.initElements(driver, this);
    }

    private int getShoeCount() {
        waitForElementsToBeVisible(listOfShoes);
        return listOfShoes.size();
    }

    private int getNumberOfShoePricesCount() {
        waitForElementsToBeVisible(listOfShoePrices);
        return listOfShoePrices.size();
    }

    private int getNumberOfShoeDescriptionsCount() {
        waitForElementsToBeVisible(listOfShoeDescriptions);
        return listOfShoeDescriptions.size();
    }

    private int getNumberOfShoeImagesCount() {
        waitForElementsToBeVisible(listOfImagesInPage);
        return listOfImagesInPage.size();
    }

    public boolean verifySeptemberPageDetails() {
        waitForElementToBeVisible(septemberShoesPage);
        if (septemberShoesPage.isDisplayed()) {
